...
class PimEnvironment(Environment):
    ...
    def upgrade(self, params: UpgradeParameters) -> list[Package | None]:
        """Upgrades Python runtimes to their latest patch versions.

        Args:
            params: Upgrade parameters containing the list of version tags to upgrade
        ...
        for name in params.names:
            ...
            try:
                ...
                if result.returncode == 0:
                    version = self.__class__._get_runtime_version(tag)
                    results.append(Package(name=name, version=version or tag))
                else:
                    logger.error(result.stderr)
                    results.append(None)
            except FileNotFoundError:
